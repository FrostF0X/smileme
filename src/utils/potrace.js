...
var Potrace = (function() {
  ...
  var imgElement = typeof document !== 'undefined' ? document.createElement("img") : null,
      imgCanvas = typeof document !== 'undefined' ? document.createElement("canvas") : null,
      bm = null,
      pathlist = [],
      callback,
      info = {
  ...
  if (imgElement) { imgElement.onload = function() { loadCanvas(); loadBm(); }; }
  ...
  function loadBm() {
    ...
    for (i = 0, j = 0; i < l; i += 4, j++) {
      ...
      bm.data[j] = (color < info.threshold ? 1 : 0);
    }
    info.isReady = true;
  }
  ...
  return{
    ...
    getPath: function() {
      var d = "";
      for (var i = 0; i < pathlist.length; i++) {
        var curve = pathlist[i].curve;
        for (var j = 0; j < curve.n; j++) {
          if (j === 0) d += "M " + curve.c[j * 3 + 2].x.toFixed(3) + " " + curve.c[j * 3 + 2].y.toFixed(3) + " ";
          else if (curve.tag[j] === "CURVE") {
            d += "C " + curve.c[j * 3 + 0].x.toFixed(3) + " " + curve.c[j * 3 + 0].y.toFixed(3) + " " +
                 curve.c[j * 3 + 1].x.toFixed(3) + " " + curve.c[j * 3 + 1].y.toFixed(3) + ", " +
                 curve.c[j * 3 + 2].x.toFixed(3) + " " + curve.c[j * 3 + 2].y.toFixed(3) + " ";
          } else {
            d += "L " + curve.c[j * 3 + 1].x.toFixed(3) + " " + curve.c[j * 3 + 1].y.toFixed(3) + " ";
            d += curve.c[j * 3 + 2].x.toFixed(3) + " " + curve.c[j * 3 + 2].y.toFixed(3) + " ";
          }
        }
        d += "Z ";
      }
