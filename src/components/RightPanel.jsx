    ...
    <div className="w-72 bg-slate-50 border-l border-slate-200 shadow-2xl flex flex-col p-4 panel-slide z-20 shrink-0 overflow-y-auto">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-4">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setTransformTarget('selection')} disabled={!activeShape || activeTool !== 'select'} className={`flex-1 text-xs py-2 font-medium rounded-lg transition-colors ${transformTarget === 'selection' ? 'bg-amber-500 text-white shadow-sm' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 disabled:opacity-50'}`}>Figura</button>
        </div>
      </div>
      {bgImage.url && (
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-4">
      ...
      {activeTool === 'select' && activeShape && (
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-4 mt-4">
          <h4 className="text-xs font-bold text-amber-600 uppercase tracking-wider mb-4 flex items-center justify-between gap-2">
            Przekształcenia
            <button onClick={() => setTransformTarget('selection')} className={`text-[10px] px-2 py-1 rounded-full ${transformTarget === 'selection' ? 'bg-amber-500 text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>Gest (2 palce)</button>
          </h4>
